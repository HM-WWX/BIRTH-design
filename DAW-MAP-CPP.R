...
DAW_MAP_CPP <- function(trt.effect_f,mu.cur_f,mu.history_f){
# 设置并行集群，使用3个核
num_cores <- 30
...
spike = 100
 ...
 results <- foreach(n = 1:ssize,.combine= "c", .packages = c('psrwe', "do","dplyr",'MASS', 'boot',
  ...
  tryCatch({   
    ...
    setwd("//home//user2//my_renv_project")
      ...
      DAW_MAP.fit = function(tau.init, target.ESS, std_heterogeneity,
        ...
        while(stop==FALSE){
          ...
          for(i in 1:n.strata){
            ...
            model <-
              jags.model(
                file = "//home//user2//my_renv_project//NEW-WWX.txt",
                data = dataTemp,
                n.chains = 1,
                n.adapt = 5000,
                quiet = TRUE,
...
##############################Treat group CPP###################################
  ...
  con.CPP[2, ] = con.numbers
    ...
    model_CPP_trt <-
      jags.model(
        file = "//home//user2//my_renv_project//HDist_3.txt",
        data = dataTemp_trt,
        n.chains = 1,
        n.adapt = 8000,
        quiet = TRUE,
...
################################Control group CPP###############################    
    ...
    Prior.theta_con[q] <- Hist.Prior.theta[q]
    Prior.sd.theta_con[q] <- Hist.Prior.sd.theta[q] 
    ############################################################################
...
if (length(null_indices) > 0) {
  ...
  # rowSums(null_decisions) > 0 意味着这一行里至少有一个亚组犯了 I 类错误
...
# 3. 为了保持结果格式一致（方便后续 rbind 和写入 Excel），
# 我们将这个单一数值复制 n.subtrial 次，形成一个向量
...
DAW_MAP_CPP_results_1 <- createWorkbook()
DAW_MAP_CPP_results_2 <- createWorkbook()
DAW_MAP_CPP_results_3 <- createWorkbook()
DAW_MAP_CPP_results_4 <- createWorkbook()
DAW_MAP_CPP_results_5 <- createWorkbook()
#Scenario 1
Scenario_1 <- DAW_MAP_CPP(trt.effect <- c(0.9808293,0.9808293,0.9808293,0.9808293,0.9808293),
...
#Scenario 2
Scenario_2 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
...
#Scenario 3
Scenario_3 <- DAW_MAP_CPP(trt.effect <- c(0.9808293,0.8472979,0.8109302,0.8472979,0.9808293),
...
#############################################################################################
addWorksheet(DAW_MAP_CPP_results_1, "Sheet1")
writeData(DAW_MAP_CPP_results_1, "Sheet1", x = NULL)
writeData(DAW_MAP_CPP_results_1, sheet = "Sheet1", c(Scenario_1_results,Scenario_2_results,
                                                     Scenario_3_results))
saveWorkbook(DAW_MAP_CPP_results_1, "DAW_MAP_CPP_1_output.xlsx", overwrite = TRUE)
##############################################################################################

#Scenario 4
Scenario_4 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
...
#Scenario 5
Scenario_5 <- DAW_MAP_CPP(trt.effect <- c(0.9808293,0.9808293,0.9808293,0,0),
...
#Scenario 6
Scenario_6 <- DAW_MAP_CPP(trt.effect <- c(0.9808293,0,0.8109302,0,0.9808293),
...
#############################################################################################
addWorksheet(DAW_MAP_CPP_results_2, "Sheet1")
writeData(DAW_MAP_CPP_results_2, "Sheet1", x = NULL)
writeData(DAW_MAP_CPP_results_2, sheet = "Sheet1", c(Scenario_4_results,Scenario_5_results,
                                                      Scenario_6_results))
saveWorkbook(DAW_MAP_CPP_results_2, "DAW_MAP_CPP_2_output.xlsx", overwrite = TRUE)
##############################################################################################

#Scenario 7
Scenario_7 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
                            mu.cur <- c(0.1980421,0.1980421,0.1980421,0.1980421,0.1980421), 
...
#Scenario 8
Scenario_8 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
                            mu.cur <- c(0.1980421,0.1980421,0.1980421,0.1980421,0.1980421), 
...
#Scenario 9
Scenario_9 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
                            mu.cur <- c(0.1980421,0.1210426,0.05792359,0,-0.05792359),
...
#Scenario 10
Scenario_10 <- DAW_MAP_CPP(trt.effect <- c(0,0,0,0,0),
                            mu.cur <- c(0.1980421,0.1210426,0.05792359,0,-0.05792359), 
...
#############################################################################################
addWorksheet(DAW_MAP_CPP_results_3, "Sheet1")
writeData(DAW_MAP_CPP_results_3, "Sheet1", x = NULL)
writeData(DAW_MAP_CPP_results_3, sheet = "Sheet1", c(Scenario_7_results,Scenario_8_results,
                                                     Scenario_9_results,Scenario_10_results))
saveWorkbook(DAW_MAP_CPP_results_3, "DAW_MAP_CPP_3_output.xlsx", overwrite = TRUE)
##############################################################################################
...
# 
 #Scenario 11
 Scenario_11 <- DAW_MAP_CPP(trt.effect <- c(0.5389965,0,0.8472979,1.252763,0),
 ...
 #Scenario 12
 Scenario_12 <- DAW_MAP_CPP(trt.effect <- c(3.044522,0,0.4054651,0,2.197225),
 ...
 #Scenario 13
Scenario_13 <- DAW_MAP_CPP(trt.effect = c(0, 0, 2.6026897, 0.4418328, 1.3499267),
...
#
 #############################################################################################
 addWorksheet(DAW_MAP_CPP_results_4, "Sheet1")
 writeData(DAW_MAP_CPP_results_4, "Sheet1", x = NULL)
 writeData(DAW_MAP_CPP_results_4, sheet = "Sheet1", c(Scenario_11_results,Scenario_12_results,
                                                      Scenario_13_results))
 saveWorkbook(DAW_MAP_CPP_results_4, "DAW_MAP_CPP_4_output.xlsx", overwrite = TRUE)
 ##############################################################################################
...
# # 设置: Null, 0.1, 0.2, 0.3, 0.2
 Scenario_14 <- DAW_MAP_CPP(trt.effect = c(0, 0.4054651, 0.8472979, 1.386294, 0.8472979),
...
# # 设置: 0.3 (强), 0.1, 0.1, 0.1, 0 (无效)
 Scenario_15 <- DAW_MAP_CPP(trt.effect = c(1.386294, 0.4054651, 0.4054651, 0.4054651, 0),
                         mu.cur = c(0, 0, 0, 0, 0),mu.history = c(0, 0, 0, 0, 0))
...
# # 设置: 0.1 (弱 - 重点观察对象), 0.3, 0.3, 0.3, 0.3 (全是强)
 Scenario_16 <- DAW_MAP_CPP(trt.effect = c(0.4054651, 1.386294, 1.386294, 1.386294, 1.386294),
                         mu.cur = c(0, 0, 0, 0, 0),mu.history = c(0, 0, 0, 0, 0))
 Scenario_16_results <- do.call(rbind, lapply(Scenario_16, function(x) as.data.frame(t(x))))


 #############################################################################################
 addWorksheet(DAW_MAP_CPP_results_5, "Sheet1")
 writeData(DAW_MAP_CPP_results_5, "Sheet1", x = NULL)
 writeData(DAW_MAP_CPP_results_5, sheet = "Sheet1", c(Scenario_14_results,Scenario_15_results,
                                                      Scenario_16_results))
 saveWorkbook(DAW_MAP_CPP_results_5, "DAW_MAP_CPP_5_output.xlsx", overwrite = TRUE)
 ##############################################################################################
